function X = EllipsoidsPropagate(system, prevX, u, d)
% EllipsoidsPropagate - Function encapsulating the propagation phase of a
...
% 
% Syntax:  
%    X = EllipsoidsPropagate(system, prevX, u, d)
%
% Inputs:
...
%
% Outputs:
%    X - Ellipsoid that X = {Ax + Bu + Ld: x \in prevX, d \in D} given u
%    value.
%
% Example: 
%    radius = 2;
...
%    X = EllipsoidsPropagate(system, X, ones(2,1), D)
%
...
X = EllipsoidsLinMap(system.A,prevX,system.B*u); % A*x + B*u
D = EllipsoidsLinMap(system.L,d,zeros(size(system.L,1),1));
X = EllipsoidsMinkowskiSum(X,D); % A*x + B*u + L*Disturbance
end
