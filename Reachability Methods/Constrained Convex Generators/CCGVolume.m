function vol = CCGVolume(X)
% CCGVolume - Function that calculates the hypervolume of the Constrained
% Convex Generator.
% 
% Syntax:  
%    vol = CCGVolume(X)
...
if n == 1
    ...
    if n <= 3
        ...
        disp(['CCGVolume G is ',num2str(size(X.G,1)),'x',num2str(size(X.G,2)),', A is ',num2str(size(X.A,1)),'x',num2str(size(X.A,2))]);
        vol = VolumeRayShooting(F,p,n,CCGCenter(X));
    end
