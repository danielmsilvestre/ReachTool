function X = AHPropagate(system, prevX, u, d)
% AHPropagate - Function encapsulating the propagation phase of a
...
% 
% Syntax:  
%    X = AHPropagate(system, prevX, u, d)
%
% Inputs:
...
%
% Outputs:
%    X - A-H polytope that X = {Ax + Bu + Ld: x \in prevX, d \in D} given u
%    value.
%
% Example: 
%    m = 10;
...
%    X = AHPropagate(system, X, ones(2,1), D)
%
...
X = AHLinMap(system.A,prevX,system.B*u); % A*x + B*u
D = AHLinMap(system.L,d,zeros(size(system.L,1),1));
X = AHMinkowskiSum(X,D); % A*x + B*u + L*Disturbance
end
