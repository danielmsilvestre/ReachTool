function X = intervalPropagate(system, prevX, u, d)
% intervalPropagate - Function encapsulating the propagation phase of a
...
% 
% Syntax:  
%    X = intervalPropagate(system, prevX, u, d)
%
% Inputs:
...
%
% Outputs:
%    X - Interval that X = {Ax + Bu + Ld: x \in prevX, d \in D} given u
%    value.
%
% Example: 
...
%    X = intervalPropagate(system, X, ones(2,1), D)
%
...
X = intervalLinMap(system.A,prevX,system.B*u); % A*x + B*u
D = intervalLinMap(system.L,d,zeros(size(system.L,1),1));
X = intervalMinkowskiSum(X,D); % A*x + B*u + L*Disturbance
end
